...
class Store:
    def __init__(self, index_name: str = "test2", search_kwargs: str = {"k": 3}, search_type: str = "similarity"):
        ...
        logger.info(f"Store initialized with index_name={index_name}, search_kwargs={search_kwargs}")
    ...
    def getRetriever(self, embeddings):
        try:
            logger.info(f"Creating retriever from Pinecone index '{self.index_name}'...")
            ...
            logger.info(f"Successfully created retriever with search_kwargs={self.search_kwargs}")
            return retriever
        except Exception as e:
            logger.error(f"Error creating retriever from Pinecone: {e}")
            logger.error("This may mean the database doesn't contain relevant documents")
