...
import type { IFormTypeAuth, IFormTypeLoginAdmin } from "@/types/form";
...
export default function useLogin() {
  ...
  const handleLogin = async (body: IFormTypeLoginAdmin) => {
    setIsLoading(true);

    // Create login data with possible OTP code
    const loginCredentials = {
      trigger: ETriggerCredentials.LOGIN_ADMIN,
    ...
    const res = await authCredential<IFormTypeLoginAdmin>(loginCredentials);

    const error = JSON.parse(res?.error || "{}");
    if (res?.error) {
      Object.keys(error || {}).forEach((key) => {
        toast.error(error?.[key]);
        form.setError(key as keyof IFormTypeLoginAdmin, {
          message: error?.[key],
        });
      });
