...
import { File, Upload, Warning, X } from "@phosphor-icons/react";
...
export default function ModalUploadFile({
  ...
  const acceptFormat = acceptedFileTypes.reduce(
    (acc, type) => {
      acc[type] = [];
      return acc;
    },
                ...
                <>
                  ...
                  >
                    ...
                    <div className="flex flex-col items-center gap-2 py-8">
                      <Upload
                        size={36}
                        className={`${hasFileSizeError ? "text-danger-500" : "text-default-400"}`}
                      />
                      <p className="mt-2 text-center text-lg font-medium">
                        Drag and Upload files here
                      </p>
                      <p className="text-center text-sm text-default-500">
                        Upload {getFileTypeDisplay()} types to expand your data.
                        Max file size upto {maxFileSize} MB.
                      </p>
                    </div>
                  </div>

                  {error && (
                    <div className="mt-4 flex items-start gap-2 rounded-md border border-danger-200 bg-danger-50 p-3">
                      <Warning
                        size={20}
                    ...
                    <div className="mt-4">
                      ...
                      <div className="max-h-[200px] space-y-2 overflow-y-auto">
                        {files.map((file, index) => (
                          <div
                            key={`${file.name}-${index}`}
                            className="flex items-center justify-between rounded-md bg-default-50 p-2"
                          >
