...
export class CreateLLMConfigDto {
    ...
    @ApiProperty({ description: 'Maximum tokens for LLM response', default: 2048 })
    @IsNumber()
    @Min(1)
    @Max(8192)
...
export class CreateChatConfigDto {
    @ApiProperty({ description: 'Maximum context length in messages', default: 20 })
    @IsNumber()
    @Min(1)
    @Max(100)
...
export class CreateChatbotConfigDto {
    @ApiProperty({ enum: ConfigType, description: 'Config type: default or override' })
    @IsEnum(ConfigType)
    type: ConfigType;
    ...
    @ApiProperty({ type: CreateChatbotPersonalityDto, description: 'Chatbot personality settings' })
    @ValidateNested()
    @Type(() => CreateChatbotPersonalityDto)
    personality: CreateChatbotPersonalityDto;

    @ApiProperty({ type: CreateAppearanceConfigDto, description: 'Appearance configuration' })
    @ValidateNested()
    @Type(() => CreateAppearanceConfigDto)
    appearance: CreateAppearanceConfigDto;

    @ApiProperty({ type: CreateWelcomeSettingsDto, description: 'Welcome settings' })
    @ValidateNested()
    @Type(() => CreateWelcomeSettingsDto)
    welcomeSettings: CreateWelcomeSettingsDto;

    @ApiProperty({ type: CreateHumanHandoffConfigDto, description: 'Human handoff configuration' })
    @ValidateNested()
    @Type(() => CreateHumanHandoffConfigDto)
    humanHandoff: CreateHumanHandoffConfigDto;

    @ApiProperty({ type: CreateContactInfoDto, description: 'Contact information' })
    @ValidateNested()
    @Type(() => CreateContactInfoDto)
    contactInfo: CreateContactInfoDto;

    @ApiProperty({ description: 'Environment: development, production', default: 'development' })
    @IsString()
    @IsOptional()
